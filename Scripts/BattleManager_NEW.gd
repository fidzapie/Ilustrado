...
# CORE FLAG - Determines button visibility
# False = Attacker sees PASS only (start of round)
# True = Attacker sees END TURN only (cards played)
...
func apply_priority_state() -> void:
	...
	if has_priority:
		...
		if is_my_attacking_phase:
			...
			# DEFENDER LOGIC - Always PASS only
			end_turn_button.visible = false
			end_turn_button.disabled = true
			pass_button.visible = true
			pass_button.disabled = false
